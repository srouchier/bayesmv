functions {
  ...
  real elec(int occ, real t, real e0_oc, vector tau_oc, real hslope_oc, 
         real cslope_oc, real e0_un, vector tau_un, real hslope_un, 
      ...
      real dly = 0;
  	  if (occ == 1) {
  	     ...
  	     dly = e0_un;
  	      if (t < tau_un[1]) {
  	          dly += hslope_un*(t - tau_un[1]);
  	      }
  	      else if (t > tau_un[2]) {
  	         dly += cslope_un*(t - tau_un[2]);
  	      }
  	 }
  	 return(dly);
  	 }
...
parameters {
  ...
  ordered[2] tau_un;    // heating and cooling breakpoints, unoccupied days
